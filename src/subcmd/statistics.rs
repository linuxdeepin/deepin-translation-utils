...
use crate::trfile::{self, common::{MessageStats, TrFileKind}};
...
pub enum CmdError {
    ...
    LoadTsFile(PathBuf, #[source] trfile::linguist::TsLoadError),
    #[error("Fail to load Gettext PO/POT file {0:?} because: {1}")]
    LoadPoError(PathBuf, #[source] trfile::gettext::PoLoadError),
...
fn load_file_stats(file_path: &Path) -> Result<MessageStats, CmdError> {
    let kind = trfile::common::TrFileKind::from_ext_hint(&file_path)
        .map_err(|e| CmdError::GuessTrFileType(file_path.to_path_buf(), e))?;

    Ok(match kind {
        TrFileKind::Linguist => trfile::linguist::Ts::load_from_file(&file_path)
            .map_err(|e| CmdError::LoadTsFile(file_path.to_path_buf(), e))?
            .get_message_stats(),
        TrFileKind::Gettext => trfile::gettext::Po::load_from_file(&file_path)
