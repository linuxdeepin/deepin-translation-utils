...
use zhconv::zhconv;
use crate::trfile::{self, linguist::Ts, gettext::Po};
...
pub enum CmdError {
    ...
    GuessTrFileType(PathBuf, #[source] trfile::common::UnknownTrExtError),
    #[error("The translation file type of target file and reference file is mismatched.")]
    MismatchedTrFileType,
    #[error("Fail to load source file {0:?} because: {1}")]
    LoadTsSourceFile(PathBuf, #[source] trfile::linguist::TsLoadError),
    #[error("Fail to load source file {0:?} because: {1}")]
    LoadPoSourceFile(PathBuf, #[source] trfile::gettext::PoLoadError),
    #[error("Fail to load target file {0:?} because: {1}")]
    LoadTsTargetFile(PathBuf, #[source] trfile::linguist::TsLoadError),
    #[error("Fail to load target file {0:?} because: {1}")]
    LoadPoTargetFile(PathBuf, #[source] trfile::gettext::PoLoadError),
    #[error("Fail to save file {0:?} because: {1}")]
    SaveTsFile(PathBuf, #[source] trfile::linguist::TsSaveError),
    #[error("Fail to save file {0:?} because: {1}")]
    SavePoFile(PathBuf, #[source] trfile::gettext::PoSaveError),
}
...
fn translate_ts_content(source_content: &Ts, target_content: &mut Ts) -> Result<(), CmdError> {
    use trfile::linguist::TranslationType;
...
impl ZhConvFile {
    fn load_file(file_path: &Path) -> Result<Self, CmdError> {
        use trfile::common::TrFileKind;
        // Detect translation file kind from given file extension.
        let tr_file_kind = TrFileKind::from_ext_hint(file_path)
            .map_err(|e| CmdError::GuessTrFileType(file_path.to_path_buf(), e))?;
        // Dispatch loading request.
        Ok(match tr_file_kind {
            TrFileKind::Linguist => Self::Linguist(
                Ts::load_from_file(file_path)
                    .map_err(|e| CmdError::LoadTsSourceFile(file_path.to_path_buf(), e))?,
            ),
            TrFileKind::Gettext => Self::Gettext(
    ...
    fn translate_content_based_on(&mut self, reference_content: &Self) -> Result<(), CmdError> {
        match (self, reference_content) {
            ...
            _ => Err(CmdError::MismatchedTrFileType)
        }
    }
...
mod tests {
    ...
    fn tst_translate_ts_content() {
        use crate::trfile::linguist::Ts;
        use crate::trfile::linguist::tests::TEST_ZH_CN_TS_CONTENT;
    ...
    fn tst_translate_po_content() {
        use crate::trfile::gettext::Po;
        use crate::trfile::gettext::tests::TEST_ZH_CN_PO_CONTENT;
